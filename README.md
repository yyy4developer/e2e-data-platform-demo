...
> **注**: Databricks App（`e2e-experiment-app`）は Serving Endpoint 依存のため、この段階ではデプロイしません（Step 10 で作成）。
> リポジトリ内の `app/` 配下のファイルは bundle sync で workspace にアップロードされます（Step 10 の source として利用）。
